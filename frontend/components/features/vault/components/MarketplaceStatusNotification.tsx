// Support both marketplace and purchase statuses
type NotificationStatus = 'idle' | 'processing' | 'success' | 'error' | 'loading_kiosk';

interface MarketplaceStatusNotificationProps {
...
export const MarketplaceStatusNotification = ({
  ...
  txDigest,
  action,
}: MarketplaceStatusNotificationProps) => {
  if (status === 'idle') {
    return null;
  }
  ...
  return (
    <div className="fixed bottom-4 right-4 z-50">
      ...
      >
        ...
        >
          <p className="text-[10px] text-white/40 uppercase tracking-widest">KIOSK MARKETPLACE</p>
        </div>
        
        {/* Ticket Content */}
        <div className="px-4 py-3 space-y-2">
          {(status === 'processing' || status === 'loading_kiosk') && (
            <div className="flex items-center gap-2">
              ...
              <span className="text-white/90 text-xs uppercase tracking-wider">
                {status === 'loading_kiosk' ? 'LOADING KIOSK' : 
          ...
          {status === 'error' && (
            <div className="space-y-1">
              ...
              {error && <p className="text-white/60 text-[10px] leading-tight">{error}</p>}
            </div>
          )}
        </div>
      </div>
