module archimeters::archimeters {
    ...
    fun init(otw: ARCHIMETERS, ctx: &mut TxContext) {
        ...
        display.add(b"name".to_string(), b"{username}".to_string());
        display.add(b"link".to_string(), b"https://archimeters.vercel.app".to_string());
        display.add(b"description".to_string(), b"Your access to the parametric design world".to_string());
        display.add(b"image_url".to_string(), b"https://aggregator.walrus-testnet.walrus.space/v1/blobs/d3ElFh07L2aZFHgFASwsIkn_WHnjnNHSz0U8SlbTi_Q".to_string());
    ...
    public fun mint_membership(
        ...
        let member = MemberShip {
            ...
            registered_time: clock::timestamp_ms(clock),
        };
        ...
        event::emit(New_member { member_id: id_copy, username });
    }

    public fun owner(membership: &MemberShip): address { membership.owner }
    ...
    public fun new_state_for_testing(ctx: &mut TxContext) {
        transfer::share_object(State { id: object::new(ctx), registered_users: table::new(ctx) });
    }
}